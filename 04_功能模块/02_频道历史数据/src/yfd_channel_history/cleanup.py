...
@dataclass(frozen=True, slots=True)
class VideoAnalyticsDayCleanupResult:
    analytics_day: str
...
class VideoAnalyticsDayCleaner:
    """备份并精确删除一个日期的全部逐视频 Analytics 记录。"""
        ...
        *,
        ...
        if expected_count is not None and len(candidates) != expected_count:
            raise ConfigurationError(
                "安全检查未通过："
                f"预期找到 {expected_count} 条视频日统计，实际找到 {len(candidates)} 条；"
                "未修改飞书数据。"
            )
        if apply:
            ...
            for batch in _batches(
                ...
                self.gateway.batch_delete_records(self.app_token, table.table_id, batch)
        else:
            bindings_deleted = 0
        return VideoAnalyticsDayCleanupResult(
