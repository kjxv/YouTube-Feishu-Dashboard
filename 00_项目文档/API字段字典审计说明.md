...
> 频道历史模块的当前业务表采用更严格的口径：只保留太平洋时间的真实 API 获取时间，
> 并用 `数据日期（太平洋时间）` 保存 Data 采集日或 Analytics 官方 `day`。
> 该模块不再写入北京时间列，也不再把日期精度的 `day` 人为扩写成截止时刻。
> 下述双时区字段是公共目录的历史兼容能力，不代表频道历史模块仍在使用。
