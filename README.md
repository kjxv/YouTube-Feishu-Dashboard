...
- 播放节点：视频追踪主表自动写入发布后 1、3、6、12、24、48、72 小时附近的真实快照播放量，并同时记录实际样本分钟数和样本时间；没有合格快照时保持空白，不补造。
...
1. 安装 Python 3.11，并确认安装时包含 Python Launcher。
2. 从 GitHub 下载项目到固定目录；路径确定后不要随意移动。
3. 双击 `1.首次配置.bat`，创建虚拟环境、安装程序并填写 `.env`。
4. 在飞书开放平台为企业自建应用开通多维表格权限、发布应用，并把应用加入目标多维表格。
5. 将 Google OAuth 桌面应用文件放到 `secrets/youtube_client_secret.json`。
6. 双击 `2.YouTube授权.bat` 完成 YouTube 授权，生成 `secrets/youtube_token.json`。
7. 双击 `3.初始化飞书配置中心.bat`，幂等创建或补齐公共配置表。
8. 双击 `5.系统检查.bat`，然后依次运行 `9.只读检查_三表配置.bat` 和
   `8.只读预览_最新视频.bat`。
9. 只读检查通过后运行 `6.运行一次_最新视频.bat`，核对飞书三张视频追踪表。
10. 运行 `11.只读检查_频道数据.bat`；首次启用频道模块时运行
    `15.启用并首次同步频道数据.bat`，普通人工补跑使用 `12.运行一次_频道数据.bat`。
11. 所有真实数据核对无误后，运行 `10.自动定时任务管理.bat`，选择“安装或更新”。

#### Windows 从旧电脑迁移

1. 在旧电脑运行 `10.自动定时任务管理.bat` 暂停任务，并确认当前没有任务正在执行。
2. 安全复制 `.env`、`secrets/youtube_client_secret.json`、
   `secrets/youtube_token.json` 和 `data/yfd.db`；不要经过 GitHub 或公开网盘。
3. 在新电脑下载相同项目，把上述文件放回相同相对路径。
4. 运行 `14.运行电脑升级并验收.bat`，再分别运行 `6` 和 `12` 做真实测试。
5. 核对飞书无误后，在新电脑运行 `10` 安装任务；旧电脑继续保持暂停。
...
VPS 统一调度器按每个整点唤醒一次。服务器控制台如果使用 UTC，`07:00 UTC` 就是北京时间
`15:00`；飞书状态字段仍按北京时间显示。时区只能造成整小时差，不会把整点变成 31 分。
