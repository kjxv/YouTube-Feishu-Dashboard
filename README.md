...
4. 运行 `yfd doctor`，再运行 `yfd scheduler run-once latest-video-tracker`。
...
`--dry-run` 不访问外部服务，会输出请求规划和缺失的引导配置。当前实施进度以 [开发进度与断点](00_项目文档/开发进度与断点.md) 和 `project_state.json` 为准。

## 当前模块状态

| 模块 | 状态 | 说明 |
|---|---|---|
| [最新发布视频实时追踪](04_功能模块/01_最新发布视频实时追踪/README.md) | 已实现 V1 | 真实快照、增量、7 天窗口、飞书幂等时间桶 |
