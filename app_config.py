...
def validate_config(config: dict[str, Any], require_complete: bool = True) -> list[str]:
    ...
    for key, label in (
        ...
        ("backfill_chunk_days", "全历史回填分段天数"),
        ("current_interval_minutes", "current 更新间隔"),
        ("latest_interval_minutes", "最新视频追踪间隔"),
        ("latest_tracking_hours", "最新视频追踪时长"),
    ):
        try:
...
def load_config(
    ...
    try:
        ...
        config["update"]["backfill_chunk_days"] = int(
            config["update"]["backfill_chunk_days"]
        )
        config["update"]["current_interval_minutes"] = int(
            config["update"]["current_interval_minutes"]
        )
        config["update"]["latest_interval_minutes"] = int(
            config["update"]["latest_interval_minutes"]
        )
        config["update"]["latest_tracking_hours"] = int(
            config["update"]["latest_tracking_hours"]
        )
    except (TypeError, ValueError):
        pass
